#include <iostream>
#include <iomanip>//giriþ-çýkýþ birimlerinin belirli formatlarda yazdýrma fonksiyonlarýný içeren bir kütüphanedir. 
#include <cstring>//Karakter dizileri ile ilgili fonksiyon, veri türü ve makro tanýmlamalarý içerir.

using namespace std;
#define MAX 1000 //Girdi olarak en fazla 1000 tane telefon bilgisi girilebilir.
struct cep_telefonu{//Bir cep telefonunun bilgilerini saklayabilen bir yapýdadýr. Cep telefonunun bütün özellikleri maksimum 10 karekterlidir.
	char marka[10];
	char model[10];
...
void puan_hesapla(struct cep_telefonu *telefonlar, int n){//Parametre olarak gelen cep telefonu dizisini ve bu dizinin uzunluðunu kullanarak her bir cep telefonunun 
//fiyat performansýný hesaplayan fonksiyondur.
	...
	for (int i=0; i<n; ++i){
	   ...
	   ih=0;
		if(strncmp(telefonlar[i].kalite, "kotu",10)==0)//telefonlar[i].kalite'nin iþaret ettiði karekter  ile kotu karekterinin en fazla 10 parametre
		//deðeri karþýlaþtýrýlýrve ayný ise 0 deðeri döndürür. strncmp fonksiyonun ile bunu yapar.
		ka=0;
...
void puana_gore_sirala(struct cep_telefonu *telefonlar, int n){//Parametre olarak gelen cep telefonu dizisini ve bu dizinin uzunluðunu kullanarak bu diziyi
//cep telefonlarýnýn fiyat performans deðerine göre büyükten küçüðe doðru sýralayan fonksiyondur.
